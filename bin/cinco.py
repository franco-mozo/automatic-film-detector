...
def cut_frame(I, superior, inferior, izquierdo, derecho):
    ...
    frame = I[superior : inferior, izquierdo : derecho, : ]
...
def main(film_number):
    ...
    # alineacion de la pelicula y escritura a memoria
    for file in files:
        ...
        aligned_frame = rotate_n_translate(frame, corn_matrix, tmp_oriented_corners, ang_tmp)     # roto y translado
        ##--- Tengo en aligned_frame el cuadro alineado sin cortar ni bordes detectados ----##

        if os.path.exists('./imagenes/' + carpeta + '/movie/' + file):
            os.remove('./imagenes/' + carpeta + '/movie/' + file)

        cv2.imwrite('./imagenes/' + carpeta + '/movie/' + file, aligned_frame)   # escribo a memoria las detectadas
    ...
    for file in files:
        frame = cv2.imread('./imagenes/' + carpeta + '/movie/' + file, 1)
        try:
    ...
    if not os.path.exists('./imagenes/' + carpeta + '/cropped/'):
       os.makedirs('./imagenes/' + carpeta + '/cropped/')
    ...
    for file in files:
        ...
        aligned_frame = rotate_n_translate(frame, corn_matrix, tmp_oriented_corners, ang_tmp)     # roto y translado
        ##--- Tengo en alined_frame el cuadro alineado sin cortar ni bordes detectados ----##

        frame = cut_frame(aligned_frame, superior, inferior, izquierdo, derecho)
